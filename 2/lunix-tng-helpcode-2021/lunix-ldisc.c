 ...
 */
 //”ακούει” τα δεδομένα που στέλνουν οι physicals drivers
 ...
 */
 /* Περνάει τα bytes που λαμβάνει στο πρωτόκολλο που χρησιμοποιούμε για να αποκωδικοποιήσουμε τα bytes σε πληροφορίες σχεδόν human readable
  * (πρέπει να μεσολαβήσει η αντιστοίχιση απο τους πίνακες του lookup tables.h)
 */
static void lunix_ldisc_receive(struct tty_struct *tty, const unsigned char *cp, char *fp, int count){
#if 1
#if LUNIX_DEBUG
	int i;
	 ...
	 */
	 /* Κάνει parse τα δεδομένα φιλτράροντας τα με τις κατάλληλες συναρτήσεις που περιγράφει το πρωτόκολλο
	  * Στο τέλος καλεί την συνάρτηση lunix_protocol_update_sensors(), η οποία με τη σειρά της καλεί την lunix_sensor_update(),
		* η οποία ενημερώνει τα πεδία του sensor struct με τα νέα δεδομένα κρατώντας το spinlock
	 */
