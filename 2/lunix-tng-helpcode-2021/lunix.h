...
/* Τα δεδομένα που προέρχονται από τον σταθμό βάσης αφού ερμηνευθούν από το κατάλληλο πρωτόκολλο αποθηκεύονται σε προσωρινούς buffers
 * Κάθε τέτοιος buffer αντιστοιχεί σε έναν συγκεκριμένο αισθητήρα
 */
 ...
 */
 //είναι διαθέσιμα σε οποιοδήποτε κομμάτι του driver αλλά ταυτόχρονα είναι τα πιο επιρρεπή σε σφάλματα τύπου race conditions
 //μας επιτρέπει να ασχοληθούμε με το τι θα συμβεί όταν κάνουμε load το module μας στον πυρήνα
#define LUNIX_SENSOR_CNT			16
extern int lunix_sensor_cnt;
extern struct lunix_sensor_struct *lunix_sensors; //πίνακας από 16 pointers που ο καθένας αντιστοιχεί στην παραπάνω δομή για τον κάθε αισθητήρα
...
//ενημερώνει τα πεδία του sensor struct με τα νέα δεδομένα κρατώντας το spinlock
